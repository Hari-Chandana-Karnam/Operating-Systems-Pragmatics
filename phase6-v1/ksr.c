...
void SysExit(void)
{
	...
	if(WAIT != pcb[parentPID].state)
    {
		...
		if(*(int *)pcb[parentPID].signal_handler[SIGCHLD] == SIGCHLD)	// Check if parent registered SIGCHLD or not.
...
{                                                                                                                                                                         
	...
	pcb[run_pid].signal_handler[signal_name] = (func_p_t) syscall;
}
...
void AlterStack(int pid, func_p_t p)
{
	...
	tempEFL = (int *) temporaryTF.eip;				
	temporaryTF.eip = *(int *) p;			// Replacing EIP in trapframe with 'p'
