...
void SysFork(void) 
{
    ...
    MemCpy((char *) (DRAM_START + pid*STACK_MAX), (char *) (DRAM_START + run_pid*STACK_MAX), STACK_MAX);    // <--Do not know what to do about this guy, yet.
...
cons_printf("After 2");
    ...
    pcb[pid].tf_p = (tf_t *)(pcb[pid].tf_p + distance);	
...
cons_printf("After 3");
    ...
    initBP = (int *) pcb[run_pid].tf_p->ebp;	//initBP gets the value of BP of INIT which is in Parent process
    initBP = initBP + distance;

cons_printf("After 4");
   //after getting the value, we put it in child process's BP.
