...
void Init(void) {  // Init, PID 1, asks/tests various OS services
	...
	while(1) 
	{
		...
		while(column != 70)
		{			
			sys_lock_mutex(VIDEO_MUTEX);	//VIDEO_MUTEX, run_pid, my_pid, forked_pid
...
//Case 1 - VIDEO_MUTEX, VIDEO_MUTEX = no loop, no pid = 2.
//case 2 - run_pid, my_pid = seg fault in enque.
//case 3 - run_pid, forked_pid = do not even enter.
