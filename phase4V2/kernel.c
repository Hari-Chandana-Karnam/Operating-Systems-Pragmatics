...
void BootStrap(void) {
	...
	//video_mutex[VIDEO_MUTEX].lock = UNLOCKED;	//set video_mutex to VIDEO_MUTEX
	...
	Bzero((char *) &video_mutex[VIDEO_MUTEX].suspend_que->que, sizeof(que_t)); 

   //enqueue all the available PID numbers to avail queue
   for(i = 0; i < QUE_MAX; i++)
   {
      EnQue(i, &avail_que);
	  //EnQue(VIDEO_MUTEX, &video_mutex[run_pid].suspend_que);
   }
