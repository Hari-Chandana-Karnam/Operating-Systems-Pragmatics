...
void SysLockMutex(void) {   // phase4
	int mutex_id;
   	...
   	mutex_id = pcb[run_pid].tf_p->ebx; //Be sure.

   	if(mutex_id == VIDEO_MUTEX) { //comparison options: VIDEO_MUTEX, run_pid
      	if(video_mutex[mutex_id].lock == UNLOCKED)
		{
         	video_mutex[mutex_id].lock = LOCKED;
      	} else {
			pcb[run_pid].state = SUSPEND;			
			EnQue(run_pid, *(int *)video_mutex[mutex_id].suspend_que->que);			
			run_pid = NONE;
      	}
   	} else {
      	cons_printf("Panic: no such mutex ID! syslock\n");
      	breakpoint();
  	}
}
...
void SysUnlockMutex(void) {
   	...
   	if(mutex_id == VIDEO_MUTEX) {
      	if(!QueEmpty(video_mutex[mutex_id].suspend_que))
		{
			released_pid = DeQue(video_mutex[mutex_id].suspend_que->que);
			EnQue(released_pid, &ready_que);
			pcb[released_pid].state = READY;         	
      	} else {
			video_mutex[mutex_id].lock = UNLOCKED;
      	}
   	} else {
      	cons_printf("Panic: no such mutex ID! sysunlock\n");
      	breakpoint();
  	}
}
