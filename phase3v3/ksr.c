...
void SysFork(void) 
{
    ...
    //pcb[PID].tf_p = (tf_t *) (DRAM_START + (PID+1)*STACK_MAX - sizeof(tf_t)); 	//Copying PCB from run_pid (Parent's Process ID)     //((int) pcb[run_pid].tf_p + distance);
    pcb[PID].tf_p = (tf_t *) ((int) pcb[run_pid].tf_p + distance); 					//Doesn't work
    //pcb[PID].tf_p = (tf_t *) (DRAM_START + (run_pid)*STACK_MAX - sizeof(tf_t));
    //pcb[PID].tf_p = (tf_t *) (DRAM_START + (run_pid+1)*STACK_MAX - sizeof(tf_t));			//Never ever use this.
    ...
    //MemCpy((char *) (DRAM_START + PID*STACK_MAX), (char *) DRAM_START, STACK_MAX);                            //We are able to fork once and is working.
    //MemCpy((char *) (DRAM_START + PID*STACK_MAX), (char *) (DRAM_START +run_pid*STACK_MAX), STACK_MAX);     //We run out of PID!
    //MemCpy((char *) (DRAM_START + PID*STACK_MAX), (char *) pcb[run_pid].tf_p, STACK_MAX);		      //Segmentation fault
    //MemCpy((char *) (DRAM_START + distance), (char *) DRAM_START, STACK_MAX);				      //Segmentation fault
    //MemCpy((char *) (DRAM_START + (PID+1)*STACK_MAX), (char *) DRAM_START, STACK_MAX);                      //Segmentation fault
    //MemCpy((char *) (DRAM_START - PID*STACK_MAX), (char *) DRAM_START, STACK_MAX); 			      //Segmentation fault
    ...
    //pcb[PID].tf_p->ebp = *(int *)initBP;   //Segmentation fault at this location itself
