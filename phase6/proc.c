...
void Init(void) {
	...
	if(forked_pid > 0) // If handles conditions for the parent.
	{	
		sys_sleep(1);			
		sys_kill(0, SIGCONT);	//call sys_kill() to all child processes (0) with signal SIGCONT
	...
	else	// Else handles conditions for the children
	{
		sys_sleep(1000000);
		column = 0;
...
{
	int exit_code, exit_cpid, my_pid;
	char pid_str[PROC_MAX], str[PROC_MAX];
