...
{	
	...
	{   
		pcb[run_pid].tf_p->ebx = DeQue(&kb.buffer); //get the 1st character and give it to the running process
	}
   	else
	{
		EnQue(run_pid, &kb.wait_que);    //queue the running PID to the wait queue in the KB data
   		pcb[run_pid].state = IO_WAIT;   //change the process state to IO_WAIT
   		run_pid = NONE;   //running process is NONE
	}
}
